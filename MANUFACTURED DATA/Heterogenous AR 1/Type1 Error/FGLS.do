...
forval sim = 1/`num_simulations' {
	...
	*Generating separate rho for each state
    ...
    * Load data and create treatment variable
	...
	collapse (mean) value rho state_d* time_d* treatment_states treatment_year treatment, by(state time)
	

	* Run FGLS regression
    hansen value treatment, group(state) time(time) nobc
