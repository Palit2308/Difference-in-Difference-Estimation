...
local N = 20             // Number of states. Change here to adjust for different number of states
...
forval sim = 1/`num_simulations' {
    ...
    * Continue generating the AR(1) process for time > 1
	...
	gen outcome = value
	replace outcome = value*1.05 if treatment == 1 // Assigning effect to the dependent variable for checking power to detect real effect.

    hansen outcome treatment, group(state) time(time)   // Performing Bias Corrected FGLS.
